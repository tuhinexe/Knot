...
const viewProfileFollowersRender = async (req, res) => {
    const user = await Users.findById(req.params.profileId).populate('followers').populate('following');
    res.render("followers", {pageTitle: 'Knot - Followers', profilePicLoggedIn: user.profilePic_url, pageName: 'viewfollowers', user: user, isFollowers: true,isFollowing: false, followers: user.followers, currentUser: req.user });
};

const viewProfileFollowingRender = async (req, res) => {
    const user = await Users.findById(req.params.profileId).populate('followers').populate('following');
    res.render("followers", {pageTitle: 'Knot - Followers', profilePicLoggedIn: user.profilePic_url, pageName: 'viewfollowers', user: user, isFollowing: true,isFollowers: false, following: user.following, currentUser: req.user });
};
